      ...
      <li *ngFor="let message of messages|async;let first=first;let last=last"
        ...
        <div *ngIf="id=='all'||mode=='inbox'">
          ...
          <div>
            ...
            <div *ngIf="(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds)>43200" style="float:right;margin-top:5px;color:#999;font-size:11px;margin-right:10px;width:60px">{{(message.payload.doc.data()?.serverTimestamp?.seconds*1000)|date:'d MMM'}}</div>
            <div *ngIf="(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds)<=43200&&(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds)>3600" style="float:right;margin-top:5px;color:#999;font-size:11px;margin-right:10px;width:60px">{{(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds)*1000|date:'h'}}h</div>
            <div *ngIf="(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds)<=3600" style="float:right;margin-top:5px;color:#999;font-size:11px;margin-right:10px;width:60px">{{math.max(0,(UI.nowSeconds-message.payload.doc.data()?.serverTimestamp?.seconds))*1000|date:'m'}}m</div>
