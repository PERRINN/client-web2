...
export class LoginComponent  {
  ...
  constructor(
    ...
    this.afAuth.user.subscribe((auth) => {
      if (auth != null) {
        this.router.navigate(['profile','user',auth.uid]);
      }
    });
  }
        ...
        this.afAuth.auth.createUserWithEmailAndPassword(email, password).catch((error) => {
          ...
          this.afAuth.user.subscribe((auth) => {
            return this.afs.collection('IDs').add({
              ...
              const imageUrlThumb="https://bit.ly/3gdkgJB";
              const imageUrlMedium="https://bit.ly/3gm44Gc";
              const imageUrlOriginal="https://bit.ly/2QnT9Bj";
